...
export function listingPayload(body, slug = null) {
  ...
  if (body.video_url) payload.video_url = body.video_url;
...
export function validateListingImages(images) {
  if (!Array.isArray(images)) return "Las imágenes deben enviarse como una lista.";
  if (images.length > 5) return "Puedes publicar hasta 5 imágenes por anuncio.";

  for (const image of images) {
    const url = String(image?.url || "").trim();
    if (!url) return "Cada imagen debe tener una URL válida.";

    let parsed;
    try {
      parsed = new URL(url);
    } catch {
      return "Una de las imágenes no tiene una URL válida.";
    }

    // Keep legacy HTTPS images working while rejecting executable or local payloads.
    if (parsed.protocol !== "https:") {
      return "Solo se permiten imágenes alojadas de forma segura.";
    }
    if (/\.(svg|html?|xml)(?:$|\?)/i.test(parsed.pathname)) {
      return "Ese formato de imagen no está permitido.";
    }
  }
...
export async function replaceImages(supabase, listingId, images) {
  ...
  if (validationError) return Object.assign(new Error(validationError), { code: "INVALID_IMAGES" });
