...
export function nearestLocationFromCoordinates(latitude, longitude, maxDistanceKm = 55) {
  if (![latitude, longitude].every((value) => Number.isFinite(Number(value)))) return null;

  const nearest = locationSuggestions
    .map((location) => {
      const coordinates = locationCoordinates(location);
      return {
        location,
        distance: distanceInKm(latitude, longitude, coordinates.lat, coordinates.lng)
      };
    })
    .filter((item) => Number.isFinite(item.distance))
    .sort((first, second) => first.distance - second.distance)[0];

  return nearest && nearest.distance <= maxDistanceKm ? nearest.location : null;
}
