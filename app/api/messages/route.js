...
export async function POST(request) {
  ...
  if (isListingInquiry) {
    ...
    if (!token) {
      return NextResponse.json(
        { error: "Inicia sesión o regístrate para enviar mensajes al anunciante." },
        { status: 401 }
      );
    }
    ...
    if (userError || !verifiedUser) {
      return NextResponse.json({ error: "Tu sesión no es válida. Inicia sesión nuevamente." }, { status: 403 });
    }
  }
  ...
  if ((!effectiveSenderEmail || !effectiveSenderEmail.includes("@")) && !allowsAnonymousEmail) {
